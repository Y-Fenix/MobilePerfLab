...
实时曲线默认会做展示层平滑，目的是降低低端机短暂缺采造成的视觉抖动；CSV/JSON/HTML 仍保存原始采样点。HTML 报告中粗线为“稳定展示”，细线为“原始值”；JSON 中 `samples` 为原始值，`display_samples` 为展示层平滑值。当实时质量发现慢采样或采集异常连续出现时，工具会从“标准稳定展示”自动切到“低端机保守展示”，进一步限制 FPS、CPU 和功耗的单点跳变，报告中的 `quality.display_strategy` 会记录本次使用的展示策略。实时质量栏会根据最近窗口直接提示“采样间隔调到 1.5s/2s，优先看稳定展示”“按真实性能波动分析”等下一步动作。采样线程按固定节拍调度，单次 adb 慢命令结束后会尽量追回原采样相位，避免后续曲线持续漂移；如果低端机上把采样间隔调到 1.5s 或 2s，实时质量和报告会按新的间隔判断慢采样，不会把主动降压误判成节拍失稳。Android FPS 计数器和上下行网络这类 delta 指标会按实际计数器读取时刻计算速率，减少低端机 adb 慢命令导致的 FPS/网络速率误放大。报告中的“采样节拍”会根据样本间隔判断“节拍稳定 / 节拍波动 / 节拍失稳”，用于区分真实性能波动和采集慢命令造成的曲线波动。
