...
- 低端机友好曲线：实时窗口默认开启展示层平滑，短暂缺采不会把曲线直接拉到 0；连续慢采样或采集异常增多时会自动切到“低端机保守展示”，让 FPS/CPU/功耗趋势更稳，同时明确标注展示策略。采样线程会按固定节拍尽量回到原有采样相位。报告中的曲线会同时展示“稳定展示”和“原始值”两条线，JSON 中同时保留 `samples` 原始采样和 `display_samples` 展示层数据。
...
实时曲线默认会做展示层平滑，目的是降低低端机短暂缺采造成的视觉抖动；CSV/JSON/HTML 仍保存原始采样点。HTML 报告中粗线为“稳定展示”，细线为“原始值”；JSON 中 `samples` 为原始值，`display_samples` 为展示层平滑值。当实时质量发现慢采样或采集异常连续出现时，工具会从“标准稳定展示”自动切到“低端机保守展示”，进一步限制 FPS、CPU 和功耗的单点跳变，报告中的 `quality.display_strategy` 会记录本次使用的展示策略。采样线程按固定节拍调度，单次 adb 慢命令结束后会尽量追回原采样相位，避免后续曲线持续漂移；如果低端机上把采样间隔调到 1.5s 或 2s，实时质量和报告会按新的间隔判断慢采样，不会把主动降压误判成节拍失稳。Android FPS 计数器和上下行网络这类 delta 指标会按实际计数器读取时刻计算速率，减少低端机 adb 慢命令导致的 FPS/网络速率误放大。报告中的“采样节拍”会根据样本间隔判断“节拍稳定 / 节拍波动 / 节拍失稳”，用于区分真实性能波动和采集慢命令造成的曲线波动。
...
HTML/JSON 报告还会生成“最近窗口”，默认看最后 8 个样本，统计慢采样、采集异常、设备级网络兜底数量和 FPS 波动幅度。它会给出“趋势：性能波动 / 采集波动 / 平稳”等结论，用于解释报告末段曲线是否还能继续参考；全局质量门禁看整场，最近窗口看当前这段。
...
弱网页的“代理真实流量”会实时显示经过本机弱网代理的上下行速率、累计流量、连接数和丢弃数，并给出“弱网已生效 / 等待目标流量 / 疑似绕过代理 / 端口不可达”等命中结论。它用于确认目标场景是否真的走到了弱网代理；该数据是代理级统计，不等同于目标 App 独占网络流量。导出报告时也会记录弱网代理运行配置、真实流量快照、历史曲线、Android 代理读回状态、端口连通状态和命中评分。
