...
- 质量门禁：实时采集会按异常样本、设备级网络兜底、前台异常和慢采样给出“高可信 / 谨慎参考 / 不可信”的会话级判断，并额外显示“最近窗口”状态，用来判断当前这一小段曲线是稳定、节拍失稳、采集异常还是网络兜底。
...
HTML/JSON 报告还会生成“最近窗口”，默认看最后 8 个样本，统计慢采样、采集异常和设备级网络兜底数量。它用于解释报告末段曲线是否还能继续参考；全局质量门禁看整场，最近窗口看当前这段。
