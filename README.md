...
弱网页的“代理真实流量”会实时显示经过本机弱网代理的上下行速率、累计流量、连接数和丢弃数，并结合链路诊断给出“弱网已生效 / 等待目标流量 / 疑似绕过代理 / 代理未确认 / 端口不可达”等命中结论。它用于确认目标场景是否真的走到了弱网代理；该数据是代理级统计，不等同于目标 App 独占网络流量。导出报告时也会记录弱网代理运行配置、真实流量快照、历史曲线、Android 代理读回状态、端口连通状态和命中评分。
